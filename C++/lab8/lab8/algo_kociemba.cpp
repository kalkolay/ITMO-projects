#include "algo_kociemba.h"

#include <cstring>
#include <functional>

Kociemba::Kociemba(int threadNum)
{
    this->threadNum = threadNum;
}

template<int Size, typename PushFunc>
void Kociemba::initPermutation(int now, int8_t* v, const PushFunc& push, int avail)
{
    if (!avail)
        return push();
...
void Kociemba::init(const char*)
{
    ...
    initHeuristic<true, true>(phrase2Edges2, &Kociemba::encodePhase2Edges2, &states);
    ...
    initHeuristic<false>(phrase1Edges, &Kociemba::encodePhase1Edges, nullptr, states);
    
    states.clear();
    
    Cube cube0;
    int8_t* cube0Ep = const_cast<int8_t*>(cube0.getCornerBlock().first);
    initPermutation<8>(0, cube0Ep, [&](){ states.push_back(cube0); }, (1 << 8) - 1 );
    ...
    initPermutation<12>(4, cube0Ep, [&](){ states.push_back(cube0); },
                        ((1 << 8) - 1) << 4 );
...
int Kociemba::estimatePhase1(const Cube& c) const
{
    return std::max(
                    ...
                    std::max(
                             phrase1Eo[encodePhase1Eo(c)],
                             phrase1Co[encodePhase1Co(c)]
                             )
                    );
}
...
{
    return std::max(
                    phrase2Corners[encodePhase2Corners(c)],
                    std::max(
                             phrase2Edges1[encodePhase2Edges1(c)],
