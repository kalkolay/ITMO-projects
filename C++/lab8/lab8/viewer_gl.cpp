...
bool ViewerGl::init()
{
    ...
    window = glfwCreateWindow(600, 600, "lab8: Rubik's Cube", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return false;
...
void ViewerGl::setCube(const Cube& cube)
{
    this->cube = cube;
}
...
{
    ViewerGl* viewer = reinterpret_cast<ViewerGl*>(glfwGetWindowUserPointer(window));
...
void ViewerGl::keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ViewerGl* viewer = reinterpret_cast<ViewerGl*>(glfwGetWindowUserPointer(window));
...
void ViewerGl::mouseButtonCallback(GLFWwindow* window, int button, int action, int mods)
{
    ViewerGl* viewer = reinterpret_cast<ViewerGl*>(glfwGetWindowUserPointer(window));
...
void ViewerGl::setRotateDuration(double sec)
{
    rotateDuration = sec;
}

void ViewerGl::drawBlock(GLfloat x,
                         GLfloat y,
                         GLfloat z,
                         GLfloat s,
                         Block color,
...
void ViewerGl::drawCube(const Cube& cube)
{
    class RotateGuard
    {
        bool isRotated;
    public:
        RotateGuard(int mask, int real, GLfloat deg, GLfloat X, GLfloat Y, GLfloat Z)
        {
            if (mask == real)
            {
                isRotated = true;
